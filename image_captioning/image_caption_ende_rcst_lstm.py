...
from class_ende_rcst_lstm import *
...
def ARNet(opt):
    ...
    model.load_state_dict(torch.load(opt.rcst_model_path))
    model.cuda()
    model.train()
    ...
    if vars(opt).get('start_from', None) is not None:
        ...
        set_lr(optimizer, opt.current_learning_rate)
    ...
    for epoch in range(opt.max_epochs):
        ...
        for start, end in zip(range(0, len(train_images_names), opt.batch_size),
            ...
            criterion_input, rcst_loss = model.rcst_forward(current_feats_fc_cuda, current_gt_sents_cuda, current_gt_masks_cuda)
            ...
            time_end = time.time()
            print("idx {}  epoch {}  lr: {:.10f}  total_loss: {:.3f}  xe_loss: {:.3f}  re_loss: {:.3f}  time: {:.3f}".format(start, 
        ...
        if np.mod(epoch, 1) == 0:
            ...
            Bleu_1, Bleu_2, Bleu_3, Bleu_4, CIDEr, METEOR, ROUGE_L, SPICE = evaluate(opt.rcst_train_json_path, val_images_names, val_images_sents)
